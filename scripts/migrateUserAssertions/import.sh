...
cqlsh -e "CREATE TABLE userassertions ( key text PRIMARY KEY, value text );"

# Report
echo "records in Cassandra before the import"
cqlsh -e "SELECT count(*) FROM userassertions;"

# Import
cqlsh -e "COPY userassertions FROM '/data/tmp/qa.final.csv' WITH CHUNKSIZE=1 AND DELIMITER='\t';"
